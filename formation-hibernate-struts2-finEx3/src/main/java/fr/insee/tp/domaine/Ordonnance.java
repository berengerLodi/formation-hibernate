...
public class Ordonnance implements Comparable<Ordonnance>, Identifiable {
	...
	@ManyToMany(targetEntity = Soin.class, cascade = { CascadeType.PERSIST,
			CascadeType.MERGE })
	@JoinTable(name = "OrdonnanceSoin", joinColumns = @JoinColumn(name = "idordonnance"), inverseJoinColumns = @JoinColumn(name = "idsoin"))
